...
namespace SDU
{
    ...
    public static class SDU_WebUIClient
    {
        ...
        public abstract class WebRequestWrapper : IDisposable
        {
            ...
            protected static IList<TResponses> GetListResponce<TResponses>(string result) where TResponses : IResponses, new()
            {
                var aJson = UCL.Core.JsonLib.JsonData.ParseJson(result);
                TResponses[] aTResponses = new TResponses[aJson.Count];
                int i = 0;
                foreach (UCL.Core.JsonLib.JsonData item in aJson)
                {
                    aTResponses[i++] = UCL.Core.JsonLib.JsonConvert.LoadDataFromJson<TResponses>(item, UCL.Core.JsonLib.JsonConvert.SaveMode.Normal);
                }
                return aTResponses;
            }
            ...
            protected async ValueTask<IList<TResponses>> SendRequestAsListResponsesAsync<TResponses>()
                where TResponses : IResponses, new()
            {
                CheckDone();
            ...
            protected async ValueTask<IList<TResponses>> SendRequestAsListResponsesAsync<TRequestBody, TResponses>(TRequestBody body)
                where TRequestBody : IRequestBody
                where TResponses : IResponses, new()
            {
                CheckDone();
            ...
            public static class SdApi
            {
                public static class V1
                {
                    ...
                    public class PngInfo : WebRequestWrapper
                    {
                        ...
                        public PngInfo(string url) : base(url, Method, RequestHeaderList) { }
                        public RequestBody GetRequestBody()
                        {
                            return new RequestBody();
                        }
